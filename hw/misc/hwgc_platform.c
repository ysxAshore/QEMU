...
static void hwgc_tlb_flush(HWGCPlatformDevState *s) { memset(s->tlb, 0, sizeof(s->tlb)); }
...
static void hwgc_start_locked(HWGCPlatformDevState *s)
{
...
static void hwgc_platform_init(Object *obj)
{
    ...
    memory_region_init_io(&s->mmio, obj, &hwgc_mmio_ops, s,
                          "hwgc-platform-mmio",
                          0x1000);
...
static void hwgc_platform_unrealize(DeviceState *dev)
{
    HWGCPlatformDevState *s = HWGC_PLATFORM_DEV(dev);
...
static void hwgc_platform_class_init(ObjectClass *oc, const void *data)
{
    DeviceClass *dc = DEVICE_CLASS(oc);

    dc->realize = hwgc_platform_realize;
    dc->unrealize = hwgc_platform_unrealize;
