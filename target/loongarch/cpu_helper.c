...
static TLBRet loongarch_map_address_debug(CPULoongArchState *env,
                                          MMUContext *context,
                                          MMUAccessType access_type, int mmu_idx,
                                          int is_debug)
{
...
{
    /* Mapped address */
    return loongarch_map_address_debug(env, context, access_type, mmu_idx, is_debug);
}
...
hwaddr loongarch_cpu_get_phys_page_debug(CPUState *cs, vaddr addr)
{
    ...
    context.addr = addr;
    if (get_physical_address_debug(env, &context, MMU_DATA_LOAD,
                                   cpu_mmu_index(cs, false), 0) != TLBRET_MATCH)
    {
        return -1;
    }
